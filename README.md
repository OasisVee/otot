...
Displays all URLs stored in the history database.

#### Delete a specific URL

```bash
otot delete https://github.com/rust-lang/rust
```
...
otot prune --older-than 30d
# Remove URLs matching a pattern
otot prune --url "google.com"
```
...
```bash
# Backup your history
otot export > history_backup.json
# Restore from backup
otot import --file history_backup.json
```
...
```bash
# For bash
source <(otot completions bash)
# For zsh
otot completions zsh > /usr/local/share/zsh/site-functions/_otot
```
...
2. Searches for URLs in your history that match on:
   ...
   - intermediate segments may be skipped, but when provided, must fuzzy match in the correct _relative order_ (not all must be provided)
